...
divideSiTodosMultiplosA1 n xs = if todosMultiplosA1 n xs 
                                then Just (map (`div` n) xs)
                                else Nothing

todosMultiplosA1 n [] = True   
todosMultiplosA1 n (x:xs) | rem x n == 0 = todosMultiplosA1 n xs
                          | otherwise    = False
...
verifica f =
    divideSiTodosMultiplos 2 [6,10,4]  ==  Just [3,5,2] &&
    divideSiTodosMultiplos 2 [6,10,5]  ==  Nothing
    where divideSiTodosMultiplos = f
