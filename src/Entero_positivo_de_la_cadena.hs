...
enteroPositivoA1 :: String -> Maybe Int
enteroPositivoA1 cs 
    | cs == [] = Nothing
    | nub (map (ord) cs) `intersect` [48..57] == nub (map (ord) cs) = Just (read cs)
    | otherwise = Nothing
...
enteroPositivoA2 :: String -> Maybe Int
enteroPositivoA2 "" = Nothing
enteroPositivoA2 cs | sonDigitos = Just (read cs)
                    | otherwise  = Nothing
    where sonDigitos = and [isDigit c |c <- cs]
