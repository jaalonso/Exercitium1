...
fibsP1:: [Integer]
fibsP1 = 1:1:zipWith (+) fibsP1 (tail fibsP1)

zeckendorfP1 :: Integer -> [Integer]
zeckendorfP1 n = aux n (takeWhile (<=n) fibsP1)
    where aux n [] = []
          aux n xs
              | n == a = [n]
              | otherwise = a:(aux (n-a) ys)
              where a = last xs 
                    ys = takeWhile (<=(n-a)) (init (init xs))

-- David
-- =====

zeckendorfP2 :: Integer -> [Integer]
zeckendorfP2 0 = []
zeckendorfP2 n = x:zeckendorfP2 (n-x)
    where x = last (takeWhile (<=n) phi)
 
phi :: [Integer]
phi = 1:1:zipWith (+) (tail phi) phi
